...
import Componentnew from './compnew';
...
const ExpenseItem = (props) => {
    ...
    const clickHandlerEvent = () => {
        ...
        props.placedNameValue(props.name) //aca la idea es mandar los valores de este item hasta App y de ahi a expenseForm
        props.placedPriceValue(props.price)
        props.placedDateValue(props.date)
        setDesmontar(!desmontar)
    }
    ...
    return <Card className='expense-item'> {/* al agregar el componente Card con estilos definidos, el componente ya no soporta el className, solo lo que trae Card, y si queremos que soporte el className, tenemos que pasarlo a traves de props */}
            <ExpenseDate dateChild={new Date(props.date)}/>   
...
/* 
    const [name, setName] = useState(props.name); // en expenseName seteo un state inicial, y luego useState retorna un array de dos elementos, donde name como variable recibe el valor que se pasa por
